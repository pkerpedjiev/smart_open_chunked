...
def patch_invalid_range_response(actual_size):
    ...
    def mock_get(*args, **kwargs):
        try:
            ...
            if (
                error_response
                and error_response.get("Message") == "Requested Range Not Satisfiable"
            ):
...
class BaseTest(unittest.TestCase):
    ...
    def assertApiCalls(self, **expected_api_calls):
        ...
        patcher = mock.patch(
            "botocore.endpoint.Endpoint.make_request", new=mock_make_request
        )
        patcher.start()
        try:
            yield api_calls
...
class SeekableRawReaderTest(unittest.TestCase):
    def setUp(self):
        self._body = b"123456"
        self._local_resource = boto3.resource(
...
class CrapClient:
    ...
    def head_object(self, *args, **kwargs):
        return {"ContentLength": self._datasize}
...
class IncrementalBackoffTest(unittest.TestCase):
    ...
    def test_every_second_read_fails(self):
        ...
        reader = smart_open.s3._SeekableRawReader(
            CrapClient(b"hello"), "bucket", "key", chunk_size=1
        )
        with mock.patch("time.sleep") as mock_sleep:
...
class ReaderTest(BaseTest):
    ...
    def test_seek_start(self):
        ...
        with self.assertApiCalls(HeadObject=1, GetObject=1):
            ...
            self.assertEqual(fin.read(6), "wořld".encode("utf-8"))
    ...
    def test_seek_past_end(self):
        with self.assertApiCalls(HeadObject=1), patch_invalid_range_response(
            str(len(self.body))
        ):
            fin = smart_open.s3.Reader(BUCKET_NAME, KEY_NAME, defer_seek=True)
            seek = fin.seek(60)
    ...
    def test_read0_does_not_return_data(self):
        with self.assertApiCalls(HeadObject=1):
    ...
    def test_binary_iterator(self):
        expected = "выйду ночью в поле с конём".encode("utf-8").split(b" ")
        _resource("s3").Object(BUCKET_NAME, KEY_NAME).put(Body=b"\n".join(expected))

        with self.assertApiCalls(GetObject=1, HeadObject=1):
    ...
    def test_read_empty_file(self):
        ...
        with self.assertApiCalls(HeadObject=1), patch_invalid_range_response("0"):
            # We open the file so we make a single HEAD request to get its length
            # The file is empty, so we should not make any GET requests
            with smart_open.s3.Reader(BUCKET_NAME, KEY_NAME) as fin:
                data = fin.read()
...
class MultipartWriterTest(unittest.TestCase):
    ...
    def test_write_01(self):
        ...
        test_string = "žluťoučký koníček".encode("utf8")
    ...
    def test_write_02(self):
        ...
        with smart_open_write as fout:
            fout.write("testžížáč".encode("utf-8"))
    ...
    def test_nonexisting_bucket(self):
        expected = "выйду ночью в поле с конём".encode("utf-8")
    ...
    def test_double_close(self):
        text = "там за туманами, вечными, пьяными".encode("utf-8")
    ...
    def test_writebuffer(self):
        ...
        with tempfile.NamedTemporaryFile(mode="rb+") as f:
            with smart_open.s3.MultipartWriter(
                BUCKET_NAME, WRITE_KEY_NAME, writebuffer=f
            ) as fout:
                fout.write(contents)
...
class SinglepartWriterTest(unittest.TestCase):
    ...
    def test_write_02(self):
        """Does s3 write unicode-utf8 conversion work?"""
        test_string = "testžížáč".encode("utf-8")
    ...
    def test_nonexisting_bucket(self):
        ...
        with self.assertRaises(ValueError):
            with smart_open.s3.open(
                "thisbucketdoesntexist", "mykey", "wb", multipart_upload=False
            ) as fout:
                fout.write(expected)
    ...
    def test_writebuffer(self):
        ...
        with tempfile.NamedTemporaryFile(mode="rb+") as f:
            with smart_open.s3.SinglepartWriter(
                BUCKET_NAME, WRITE_KEY_NAME, writebuffer=f
            ) as fout:
                fout.write(contents)
...
ARBITRARY_CLIENT_ERROR = botocore.client.ClientError(
    error_response={}, operation_name="bar"
)
...
class IterBucketTest(unittest.TestCase):
    ...
    @pytest.mark.skipif(
        condition=sys.platform == "win32", reason="does not run on windows"
    )
    @pytest.mark.xfail(
    ...
    def test_iter_bucket_404(self):
        ...
        with mock.patch(
            "smart_open.s3._download_fileobj", side_effect=throw_404_error_for_key_4
        ):
            results = list(smart_open.s3.iter_bucket(BUCKET_NAME))
    ...
    def test_iter_bucket_non_404(self):
        populate_bucket()
        with mock.patch(
            "smart_open.s3._download_fileobj", side_effect=ARBITRARY_CLIENT_ERROR
        ):
    ...
    def test_intermittent_error_retries(self):
        ...
        with mock.patch("smart_open.s3._download_fileobj", side_effect=side_effect):
            actual = smart_open.s3._download_key(
                KEY_NAME, bucket_name=BUCKET_NAME, retries=4
            )
        self.assertEqual(expected, actual)
...
class RetryIfFailedTest(unittest.TestCase):
    ...
    def test_failure(self):
        ...
        with self.assertRaises(IOError):
            smart_open.s3._retry_if_failed(
                partial, attempts=3, sleep_seconds=0, exceptions=exceptions
            )
...
def test_resource_propagation_reader():
    ...
    with smart_open.s3.open(
        BUCKET_NAME, WRITE_KEY_NAME, mode="rb", client=client
    ) as reader:
